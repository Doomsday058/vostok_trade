...
Это вторая версия проекта ([vostok_trade_old_version](https://github.com/Doomsday058/vostok_trade_frontend)). Он был переписан с классической связки React+Express на **Next.js 15 (App Router)** для улучшения SEO, производительности и удобства деплоя.
...
* **👑 Админ-панель:**
    ...
    * Массовая загрузка/обновление товаров через **Excel (.xlsx)** — см. «Импорт каталога».
...
Администратор загружает каталог файлом `.xlsx` в личном кабинете. Первая строка листа —
заголовки колонок: `title` (обязательно), `price`, `description`, `image`, `details`.

Файл заменяет каталог целиком, поэтому импорт устроен так, чтобы не оставить магазин без товаров:

1. Сначала разбирается весь лист ([`lib/priceImport.ts`](lib/priceImport.ts)): пустое название,
   нечисловая или отрицательная цена — ошибка с номером строки Excel.
2. Если есть хоть одна ошибка или в файле нет товаров, каталог не трогается,
   администратор видит список проблемных строк.
3. Удаление старого каталога и вставка нового идут в одной транзакции MongoDB:
   при сбое посередине остаётся прежний каталог. Транзакции требуют replica set —
   в MongoDB Atlas это так по умолчанию.

Разбор строк покрыт тестами: `npm test`.
...
```bash
git clone https://github.com/Doomsday058/vostok_trade.git
cd vostok_trade
```
